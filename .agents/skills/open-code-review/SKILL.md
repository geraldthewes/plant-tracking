...
This project's rules include citable rule IDs for traceable feedback:

| Prefix | Source | Example |
|--------|--------|---------|
| `PY-LANG-*` | Google Python Style Guide (Language) | `PY-LANG-007` (Handle exceptions carefully) |
| `PY-STY-*` | Google Python Style Guide (Style) | `PY-STY-002` (Limit line length to 80 chars) |
| `PYSCG-*` | OpenSSF Secure Coding Guide | `PYSCG-0041` (Externalize Configuration and Secrets) |

When reviewing, cite rule IDs in findings: `file.py:42` — **PY-LANG-007** (Handle exceptions carefully)

### Structured Output Format

When the LLM's review output includes citable rule IDs, use this enhanced format:
...
### Must-Fix Issues
- **`file.py:42`** — **PYSCG-0041** (Externalize Configuration and Secrets)
  - Why: ...
  - Suggested fix: ...

### Suggestions
- **`file.py:88`** — **PY-LANG-015** (Avoid mutable default arguments)
  - Why: ...
  - Suggested fix: ...
