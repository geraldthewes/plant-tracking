...
description: Classify a single architecture-critique item and find dedup matches in knowledge/review-wiki/. Given an item (title, severity, problem text, recommendation text) and the wiki path, return a suggested action (ticket/pitfall/nit/backlog/non-issue) with rationale and any similar existing wiki entries. Read-only — never creates or edits files.
mode: subagent
...
2. Search existing wiki entries for semantic matches (dedup detection)
...
**pitfall** — when:
- The issue describes a recurring antipattern (e.g., "swallowed exceptions", "hardcoded credentials")
- The problem is structural or cultural, not a one-off fix
- It would benefit future review sessions to know this pattern exists
- Keywords: "pattern", "consistently", "across the codebase", "never", "always missing"
...
**non-issue** — when:
- The problem is acknowledged but doesn't apply to this codebase's context
- The recommendation contradicts a known project decision
- The issue is about conventions that the project intentionally doesn't follow
...
**rejected-alternative** — when: the report's verdict explicitly says "Reject" or "Reject because…".
**skip** — when: the alternative is purely theoretical with no practical path.

## Dedup Detection

Search `knowledge/review-wiki/` for similar existing entries:

```
Grep for key terms from the item title across all .md files in the wiki
Check all five sections: pitfalls/, strengths/, backlog/, non-issues/, rejected-alternatives/
```

A match is worth flagging when:
- 3+ words from the item title appear in an existing file's `topic:` frontmatter field
...
- Whether this looks like the same pattern (should append occurrence) or just a related-but-different one (new entry)
...
─────────────────────────
Item: <title>
Suggested action: ticket | pitfall | nit | backlog | non-issue | rejected-alternative | skip
Rationale: <one sentence explaining why>

DEDUP CHECK
───────────
[No existing wiki entries match this item.]
  — OR —
[Possible match: pitfalls/some-existing-entry.md]
  Topic: <existing topic>
  Overlap: <which terms overlap>
  Verdict: Likely the same pattern (suggest appending occurrence) | Related but distinct (new entry)
```

## Constraints

- Read-only: never create, edit, or delete any file
- Only use Grep and Glob for searching; use Read only for frontmatter inspection (limit first 15 lines)
- Keep output short — one paragraph max per section
- Do not generate the ticket or wiki entry — that is done by the calling command
