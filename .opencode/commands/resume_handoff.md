...
1. **If the path to a handoff document was provided**:
   ...
   - Immediately read any research or plan documents that it links to under `knowledge/plans` or `knowledge/research`. do NOT use a sub-agent to read these critical files.
   - Begin the analysis process by ingesting relevant context from the handoff document, reading additional files it mentions
   - Then propose a course of action to the user and confirm, or ask for clarification on direction.

2. **If a ticket number (like ENG-XXXX) was provided**:
   - Ensure your `knowledge/` directory is up to date.
   - locate the most recent handoff document for the ticket. Tickets will be located in `knowledge/handoffs/ENG-XXXX` where `ENG-XXXX` is the ticket number. e.g. for `ENG-2124` the handoffs would be in `knowledge/handoffs/ENG-2124/`. **List this directory's contents.**
...
Tip: You can invoke this command directly with a handoff path: `/resume_handoff `knowledge/handoffs/ENG-XXXX/YYYY-MM-DD_HH-MM-SS_ENG-XXXX_description.md`
...
2. **Spawn focused research sub-agents**:
   Based on the handoff content, spawn parallel research sub-agents to verify current state.
   Use `@knowledge-analyzer` to read and extract key insights from artifacts mentioned in the handoff.
...
1. **Create a task list** based on action items:
   - Convert action items from handoff into a numbered todo list
   - Add any new tasks discovered during analysis
   - Prioritize based on dependencies and handoff guidance

2. **Present the plan**:
   ```
   Here's my task list based on the handoff and current analysis:
